...
- `help` prints the full command list one command per line.
...
- `power status`, `power perf`, `power balanced`, and `power saver` are stable runtime controls for the wakeup-reduction subsystem.
- `sim` generates a burst of small `/net/tx` writes and then prints flush metrics.
...
- `power status`: inspect wakeup, coalescing, and queued transmit counters
- `power saver`: switch to the most aggressive batching mode
- `sim 8`: quick wakeup/flush demo using the current power mode
